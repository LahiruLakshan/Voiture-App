...
class BusLorySpareParts extends StatefulWidget {
  const BusLorySpareParts({Key key}) : super(key: key);

  @override
  _BusLorySparePartsState createState() => _BusLorySparePartsState();
}

class _BusLorySparePartsState extends State<BusLorySpareParts> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Bus / Lorry Spare Parts"),
...
class _ListState extends State<List> {
  ...
  Future getCarGarages() async {
    ...
    QuerySnapshot qn = await db.collection("provider").where("SerCat", isEqualTo: "Spare parts").where("VehiType", isEqualTo: "Bus / Lorry").get();
    return qn.docs;
  }
